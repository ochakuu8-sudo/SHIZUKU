...
すべてのマスは、停止時にイベント絵とテキストを表示します。移動時には基本的にスタミナを1消費し、`strong` ルートでは2消費します。スタミナが足りない場合は疲労ダメージを受けます。`event` マスでは `category` が `data/events.json` のイベントカテゴリと対応し、選択肢つきイベントを再生します。それ以外のマスは、マス効果を処理したうえで `description` を使った到着シーンを表示します。
