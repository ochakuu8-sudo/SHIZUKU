...
Godot 4.5 向けの、データ差し替え式すごろく育成RPGプロトタイプです。
...
文章と画像は `data/events.json` を編集します。画像は Godot が読める `res://` パスを指定してください。盤面の見た目は `data/board.json` の `width`、`height`、各マスの `x`、`y` で変えられます。
